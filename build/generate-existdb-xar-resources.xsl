...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:col-config="http://exist-db.org/collection-config/1.0" exclude-result-prefixes="xs" version="2.0">
    ...
    <xsl:template match="col-config:collection" mode="collection-xconf">
        <xsl:message>col-config:collection: <xsl:value-of select="$path"/>
        </xsl:message>
        <xsl:for-each select="$xars//app[./name=$app-name]/col-config:index">
            ...
            <xsl:result-document href="{$path}/{$filename}" exclude-result-prefixes="#all" indent="1">
